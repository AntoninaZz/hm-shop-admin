...
export const GET = async (req: NextRequest, { params }: { params: { bannerId: string } }) => {
    try {
        await connectToDB();
...
export const POST = async (req: NextRequest, { params }: { params: { bannerId: string } }) => {
    try {
...
export const DELETE = async (req: NextRequest, { params }: { params: { bannerId: string } }) => {
    try {
        const { userId } = await auth();
        if (!userId) {
            return new NextResponse("Unauthorized", { status: 401 });
