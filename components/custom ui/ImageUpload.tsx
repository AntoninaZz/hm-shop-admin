    ...
    return (
        <div>
                ...
                {value.map(((url, i) => (
                    <div key={i} className='relative w-[200px] h-[200px]'>
                        <div className='absolute top-2 right-2 z-1'>
                            <Button onClick={() => onRemove(url)} size="sm" className='bg-[var(--color-powder-pink)] text-white cursor-pointer'>
            ...
            <CldUploadWidget uploadPreset="hm-shop-admin" onSuccess={onUpload}>
                {({ open }) => {
                    return (
                        <Button onClick={() => open()} className='bg-[var(--color-light-beige)] text-white cursor-pointer'>
